...
class TransformerModel_Gazeformer(pl.LightningModule):
    def __init__(self, args, max_len):
        ...
        self.total_step = 0
        self.loss_fn_token = torch.nn.NLLLoss()
        self.loss_fn_xy = nn.L1Loss(reduction='mean')
    ...
    def train_one_dataset(self, batch, type, return_gaze=False):
        ...
        for i in range(gt.size()[1]):
            ...
            logit_gt = torch.zeros(((end_token+1))).to(DEVICE).type(torch.LongTensor)
            logit_gt[-1] = 1
    ...
    def test_step(self, batch, batch_idx):
        ...
        if len(data1) == 0: # 7, 1?
            ...
            loss_gt, GAZE = self.valid_one_dataset(data1, 1)
            GAZE_gt = data1[2][1:, :][:-1]
            GAZE = GAZE[0]
            ss = nw_matching(GAZE_gt[:, 0].detach().cpu().numpy(), GAZE[:, 0].detach().cpu().numpy())
