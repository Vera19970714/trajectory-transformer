...
class PositionalEncoding(nn.Module):
    def __init__(self,
        ...
        pos = torch.arange(0, maxlen).reshape(maxlen, 1).float() 
        pos_embedding = torch.zeros((maxlen, emb_size)).float() 
...
def gaussian_2d(x, y, x0, y0, xsigma, ysigma, A):
    return A * np.exp(-((x - x0) ** 2 / (2 * xsigma ** 2) + (y - y0) ** 2 / (2 * ysigma ** 2)))
...
def draw2Dheatmap():
    ...
    totali = 12
    totalj = 12
    size = 12  # Define the size of the matrix
    sigma = 1  # Standard deviation for the Gaussian distribution
    
    xx = np.linspace(-6, 5, size)
    yy = np.linspace(-6, 5, size)
    xx, yy = np.meshgrid(xx, yy)

    gaussian_matrix = gaussian_2d(xx, yy, 0, 0, sigma, sigma, 1)
    ...
    for i in range(totali):
        for j in range(totalj):
            ...
            # add updated version:
            order = 15
            target_sim = gaussian_matrix[i][j]
            target_sim2 = a ** order
            #print(target_sim)
            #print('--')
            #print(target_sim2)
