...
class Visualization(object):
    def __init__(self, training_dataset_choice, testing_dataset_choice, evaluation_url, gazeNum=10, plotRange=50, showBenchmark=False):
        ...
        self.shelfFolder = './dataset/amazon_data/images/'
        ...
        gaze_expect = evaluation_url+'/amazon_pamformer/gaze_expect.csv'
    ...
    def generate_patch_heatmap(self,background_img, target_pos, trajectories, num_patches_row, num_patches_col, save_path, alpha=0.5):
        ...
        # Overlay the heatmap
        im = ax.imshow(heatmap, cmap='hot', alpha=alpha, extent=(0, w, h, 0), vmin=0, vmax=0.1)

        # Create the color bar
        cax = fig.add_axes([ax.get_position().x1 + 0.01, ax.get_position().y0, 0.02, ax.get_position().height*0.98])
    ...
    def forward(self):
        ...
        for i in tqdm(range(self.data_length)):
            ...
            package_target = self.package_target[i][0]
            ...
            package_position_gt = self.get_patch_position(np.array(package_seq)-1,IMAGE_COLUMN)
            ...
            # package_position_gt， package_position_muti
            self.generate_patch_heatmap(question_img_cropped, [IMAGE_RANGE_X_target,IMAGE_RANGE_Y_target],package_position_muti,IMAGE_ROW, IMAGE_COLUMN, save_path = 'heatmap/oat2/expect' + str(i))
            self.generate_patch_heatmap(question_img_cropped, [IMAGE_RANGE_X_target, IMAGE_RANGE_Y_target],
                                        package_position_gt, IMAGE_ROW, IMAGE_COLUMN,
                                        save_path='heatmap/gt2/expect' + str(i))
...
if __name__ == '__main__':
    ...
    evaluation_url = './dataset/checkEvaluation'
